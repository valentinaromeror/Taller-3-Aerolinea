...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (ruta == null) throw new InformacionInconsistenteException("Ruta no existe: " + codigoRuta);

        Avion avion = null;
        for (Avion a : aviones)
            if (a.getNombre().equals(nombreAvion)) { avion = a; break; }
        if (avion == null) throw new InformacionInconsistenteException("Avión no existe: " + nombreAvion);

        java.util.function.Function<String,Integer> aMin = (hhmm) -> {
            String[] p = hhmm.split(":");
            return Integer.parseInt(p[0]) * 60 + Integer.parseInt(p[1]);
        };

        int iniNuevo = aMin.apply(ruta.getHoraSalida());
        int finNuevo = aMin.apply(ruta.getHoraLlegada());

        for (Vuelo v : vuelos)
        {
            if (v.getAvion().getNombre().equals(nombreAvion) && v.getFecha().equals(fecha))
            {
                int iniExist = aMin.apply(v.getRuta().getHoraSalida());
                int finExist = aMin.apply(v.getRuta().getHoraLlegada());
                boolean seSolapan = iniNuevo < finExist && iniExist < finNuevo;
                if (seSolapan)
                    throw new InformacionInconsistenteException("El avión ya está ocupado en ese horario");
            }
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (cliente == null) throw new InformacionInconsistenteException("Cliente no existe: " + identificadorCliente);

        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo == null) throw new InformacionInconsistenteException("No existe vuelo " + codigoRuta + " en " + fecha);

        String f = fecha.replace('/', '-');
        String[] p = f.split("-");
        if (p.length < 2) throw new InformacionInconsistenteException("Fecha inválida: " + fecha);
        int mes = Integer.parseInt(p[1]);
        boolean temporadaBaja = (mes >= 1 && mes <= 5) || (mes >= 9 && mes <= 11);

        CalculadoraTarifas calc = temporadaBaja
                ? new CalculadoraTemporadaBaja()
                : new CalculadoraTemporadaAlta();

        int tarifaUnidad = calc.calcularTarifa(vuelo, cliente);
        int total = tarifaUnidad * cantidad;

        vuelo.venderTiquetes(cliente, calc, cantidad);
